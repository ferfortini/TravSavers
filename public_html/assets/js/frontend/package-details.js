const data = JSON.parse(localStorage.getItem('selectedPackageHotel'));
...
if (data) displayHotelInfo(data);
fetchHotelDetails(formData(data));
...
$('#guestModal').on('hidden.bs.modal', function () {
    $('#room_id').val('');
    $('#guest-form')[0].reset(); // Reset all form fields to their default values
...
function formData(data) {
    const locationName = data?.destinationName;
    const locationCode = destinationIdMap[locationName] || data.destinationId;
    const hotelAddress = data.destinationName;
    const hotelId = data.hotelId || destinationIdMap[hotelAddress];
    return {
                    ...
                    $('#room-cards').append(`<div class="col-md-6 col-lg-4">
                                    ...
                                    <div class="card-body p-0 overflow-hidden">
                                            ...
                                            <div class="room-card-content p-3">
                                                ...
                                                <input type="hidden" name="package_id" value="${room.groupedPackages[0].packageId}">
                                                <div
...
function getPackageDetails(data) {
    ...
    $.ajax({
        type: 'POST',
        url: 'package_detail_api_call.php',
        data: { package_id: packageId },
        success: function (response) {
            if (response) {
                const jsonResponse = JSON.parse(response);
                $('#package-name').text(jsonResponse?.package_title);
                $('#package-description').text(jsonResponse?.description);

                const perks = typeof jsonResponse.include_perks === 'string'
                    ? JSON.parse(jsonResponse.include_perks)
                    : jsonResponse.include_perks;

                for (const perk of perks) {
                    const perkName = perk.replace(/\s*\(\$?\d+\)$/, '');

                    $('#include-perks').append(`<li class="d-flex gap-2">
                      <i class="bi bi-check-circle-fill"></i>
                      <span>${perkName}</span>
                    </li>`);
                }
