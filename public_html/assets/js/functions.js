...
var e = {
    init: function () {
        e.preLoader(),
            ...
            e.trafficChart(),
            e.guestSelector(),
            e.packageGuestSelector(),
            e.parallaxBG(),
            //e.overlayScrollbars(),
            e.trafficsplineChart(),
            e.trafficroomChart();
        e.trafficBookingChart();
    },
        ...
        if (e.isVariableDefined(choice)) {
            ...
            element.forEach(function (item) {
                ...
                if (item.choicesInstance) {
                    item.choicesInstance.destroy();
                    delete item.choicesInstance;
                }

                var removeItemBtn = item.getAttribute("data-remove-item-button") == "false" ? false : true;
                var placeHolder = item.getAttribute("data-placeholder") == "false" ? false : true;
                var placeHolderVal = item.getAttribute("data-placeholder-val") ?
                    item.getAttribute("data-placeholder-val") :
                    "Select one or more options";
                var maxItemCount = item.getAttribute("data-max-item-count") !== null ?
                    parseInt(item.getAttribute("data-max-item-count"), 10) :
                    -1;
                var searchEnabled = item.getAttribute("data-search-enabled") == "true" ? true : false;

                item.choicesInstance = new Choices(item, {
        ...
        if (e.isVariableDefined(picker)) {
            ...
            element.forEach(function (item) {
                ...
                flatpickr(item, {
                    ...
                    dateFormat: "m/d/Y",
                    disableMobile: true,
                    minDate: ["date_range", "package_arrival_date"].includes(item.id) ? "today" : null,
                    onChange: function (selectedDates, dateStr, instance) {
                        if (item.id === "package_arrival_date") {
                            var arrivalDateInput = document.getElementById("package_arrival_date");
                            arrivalDateInput.value = dateStr;
                        }

                        // Set minDate on the corresponding departure input
                        if (item.id === "date_range" || item.id === "package_arrival_date") {
                            var arrivalDate = selectedDates[0];
                            var departureInputId = item.id === "date_range" ? "departure_date" : "package_departure_date";
    ...
    // START: Package Guest Selector
    packageGuestSelector: function () {
        if (e.isVariableDefined(e.select(".package-guest-selector"))) {
            let adults = 2;
            let child = 0;
            let rooms = 1;
            let totalAdults = 2;

            let selectionResult = document.querySelector(".package-selection-result");

            let adultValue = document.querySelector(".package-adults");
            
            let adultAdd = document.querySelector(".package-adult-add");
            let adultRemove = document.querySelector(".package-adult-remove");

            let childValue = document.querySelector(".package-child");
            let childAdd = document.querySelector(".package-child-add");
            let childRemove = document.querySelector(".package-child-remove");

            function addPackageElement(type) {
                if (type == "adult") {
                    adults++;
                    totalAdults = adults + child;
            ...
            function removePackageElement(type) {
                if (type == "adult") {
                    adults = adults > 0 ? adults - 1 : adults;
                    totalAdults = adults + child;
            ...
            adultAdd.addEventListener("click", function () {
                addPackageElement("adult");
            });

            adultRemove.addEventListener("click", function () {
                removePackageElement("adult");
            });

            childAdd.addEventListener("click", function () {
                addPackageElement("child");
            });

            childRemove.addEventListener("click", function () {
                removePackageElement("child");
            });
        }
    },
    // END: Package Guest Selector
    ...
    trafficsplineChart: function () {
        var cpv = e.select("#ChartSpendingVsSaving");
        if (e.isVariableDefined(cpv)) {
            // CHART: Page Views
            var options = {
                series: [
                    {
                        name: "Spent",
                        data: [0, 0, 396, 0, 845, 267, 0, 598, 0, 1367, 952, 0]
                    },
                    {
                        name: "Saved",
                        data: [0, 0, 127, 0, 284, 127, 0, 90, 0, 205, 149, 0]
                    }
                ],
                chart: {
                    height: 350,
                ...
                colors: [
                    ThemeColor.getCssVariableValue("--bs-primary"),
                    ThemeColor.getCssVariableValue("--bs-emerald")
                ],
                dataLabels: {
                    enabled: false
                },
                ...
                fill: {
                    colors: ["rgba(29,47,103,0.3)", "rgba(0,208,98,0.3)"]
                }
            };

            var chart = new ApexCharts(document.querySelector("#ChartSpendingVsSaving"), options);
            chart.render();
        }
    },
    // END: Traffic Chart 3
    ...
    trafficroomChart: function () {
        var cpv = e.select("#ChartSpendingDonut");
        if (e.isVariableDefined(cpv)) {
            // CHART: Page Views
            var options = {
                series: [4425.0, 982.0],
            ...
            var chart = new ApexCharts(document.querySelector("#ChartSpendingDonut"), options);
            chart.render();
        }
    },
    // END: Traffic Chart 4

    trafficBookingChart: function () {
        var cpv = e.select("#ChartRoomsBooked");
        if (e.isVariableDefined(cpv)) {
            // CHART: Page Views
            var options = {
                chart: {
            ...
            var chart = new ApexCharts(document.querySelector("#ChartRoomsBooked"), options);
            chart.render();
        }
    }
};
